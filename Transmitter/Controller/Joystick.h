...
class Joystick{
public:
	Joystick(uint16_t x, uint16_t y, uint16_t button, uint8_t minV = 0, uint8_t maxV = 4095)
	: _x(x), _y(y), _button(button), _minV(minV), _maxV(maxV){
		pinMode(_x, INPUT_PULLDOWN);
		pinMode(_y, INPUT_PULLDOWN);
		pinMode(_button, INPUT_PULLDOWN);
	}
	uint16_t getX(){
